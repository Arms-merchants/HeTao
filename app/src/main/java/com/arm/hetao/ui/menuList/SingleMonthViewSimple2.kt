 ...
 *    version: 1.0
 */
class SingleMonthViewSimple2 @JvmOverloads constructor(
    context: Context,
    attrs: AttributeSet? = null,
    defStyleAttr: Int = 0
) : SingleMonthView(context, attrs, defStyleAttr) {
    ...
    override fun onDrawMonthBefore(canvas: Canvas, calendarDay: CalendarDay) {
        ...
        val monthBaseLine =
            ((height - paddingTop - paddingBottom) / 2 + (fm.descent - fm.ascent) / 2 - fm.descent)
        canvas.drawText(
            (calendarDay.month + 1).toString(), (width - paddingLeft - paddingRight) / 2F,
    ...
    override fun onDrawDay(
        ...
        canvas.drawText(
            ...
            rect.top + verticalOffset + abs(fm.ascent),
            dayTextPaint
        )

        val lunarText =
            LunarCalendar.getLunarText(calendarDay.year, calendarDay.month + 1, calendarDay.day)
        lunarTextPaint.getFontMetrics(fm)
        canvas.drawText(
            lunarText,
            rect.centerX().toFloat(),
            rect.bottom - verticalOffset - fm.descent,
            lunarTextPaint
        )
    }
    ...
    private fun initDrawable() {
        selectedBgDrawable =
            ContextCompat.getDrawable(context, R.drawable.drawable_calendar_circle_selected_bg)!!
    }
